...
async def submit_answers(data: AnswersRequest):
    ...
    try:
        ...
        if not session:
            # Se la sessione non esiste, potrebbe essere stata persa. Crea una nuova con dati minimi.
            # Questo non dovrebbe succedere se la sessione è stata creata in /api/questions/generate
            raise HTTPException(
                status_code=404,
        ...
        # Salva la sessione aggiornata (FileSessionStore salva automaticamente)
        if isinstance(session_store, FileSessionStore):
            session_store._save_sessions()
        
        print(f"[DEBUG] Risposte aggiornate per sessione {data.session_id}: {len(data.answers)} risposte")
        ...
        print(f"[ERROR] Errore nell'elaborazione delle risposte: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(
            status_code=500,
